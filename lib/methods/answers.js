...
/**
 * Gets the comments to a set of answers identified in [ids].
 *
 * @param {Object} criteria
 * @param {Function} callback return results
 * @param {Array} ids collection of IDs
...
function comments (criteria, callback, ids) {
  if (!ids || !ids.length) return callback(new Error('answers.comments lacks IDs to query'))
  query('answers/' + ids.join(';') + '/comments', criteria, callback)
}
